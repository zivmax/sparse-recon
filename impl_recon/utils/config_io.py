...
def get_model_config_path(
    path_config: Dict, model_name: str, config_pattern: str
) -> Path:
...
def write_config(source_config_file: Path, target_dir: Path) -> None:
    ...
    if not target_dir.exists():
        raise ValueError(
            "Target directory for writing config does not exist:\n{}".format(target_dir)
        )
...
def read_yaml_config(
    ...
    default_config_file_path: Optional[Union[Path, str]] = None,
) -> Dict:
    ...
    if default_config_file_path is not None:
        ...
        if not default_config_file_path.exists():
            raise ValueError(
                f"Default YAML config file does not exist:\n"
...
def parse_config_train() -> Tuple[Dict, Path]:
    ...
    if args.config_file is None:
        raise ValueError(
            "Training config must be provided via a command-line argument."
        )
...
def parse_config_eval() -> Tuple[Dict, Path]:
    ...
    """
    parser = argparse.ArgumentParser(
        description="Evaluate a neural network.",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    parser.add_argument("-p", "--path_file", type=str)
    ...
    if args.path_file is None or args.model_name is None or args.eval_config is None:
        raise ValueError(
            "Path config, evaluation config and model name must be provided."
        )
    ...
    if config["model_name"] != args.model_name:
        raise ValueError(
            f'Cannot load stored model "{args.model_name}": it contains a different '
