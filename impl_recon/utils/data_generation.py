...
def downsample(
    image: torch.Tensor, target_spatial_shape: List[int], do_threshold: bool
) -> torch.Tensor:
...
def load_volumes(
    volumes_dir: Path, casenames: List[str]
    ...
    for casename in casenames:
        ...
        if len(files) != 1:
            raise ValueError(
                f"Exactly one file must fit the pattern:\n"
...
def compute_thick_slices(
    volume: torch.Tensor, axis: int, step_size: int
) -> torch.Tensor:
    ...
    """
    thick_slices = []
    max_num_slices_below = (
        round(step_size / 2) - 1 if step_size % 2 == 0 else round(step_size / 2 - 0.5)
    )
...
def select_fullres_subset(num_cases_total: int, fraction: float) -> List[int]:
    ...
    caseids = torch.multinomial(
        torch.ones(num_cases_total), num_cases_actual, False, generator=gen_subset
    ).tolist()
    return caseids


def split_train_val_caseids(
    num_cases: int, val_fraction: float
    ...
    if val_set_size == 0:
        raise ValueError(
            f"Empty validation set (fraction {val_fraction} of {num_cases} cases)."
        )
    if val_set_size == num_cases:
        raise ValueError(
            f"Empty training set (fraction {val_fraction} of {num_cases} cases)."
        )
...
def split_train_val_casenames(
    casenames: List[str], val_fraction: float, is_training: bool
) -> List[str]:
...
def generate_mid_slice_ids(label: torch.Tensor) -> torch.Tensor:
    ...
    for i in range(3):
        ...
        individual_voxel_ids_meshed = torch.meshgrid(
            individual_voxel_ids, indexing="ij"
        )
        individual_voxel_ids_meshed_flat = [
            torch.flatten(x) for x in individual_voxel_ids_meshed
        ]
        slice_voxel_ids = torch.empty(
            (shape[axis1] * shape[axis2], 3), dtype=torch.int64
        )
...
class ReconNetDataset(data.Dataset):
    def __init__(
        ...
        if do_deterministic_sparsing:
            ...
            self.slice_start_ids = torch.randint(
                0, slice_step_size, [num_cases_original], generator=gen
            ).tolist()
    ...
    def __getitem__(self, item: int):
        ...
        if self.crop_size > 0:
            ...
            crop_start_x = torch.randint(
                0, label_hr.shape[0] - self.crop_size + 1, [1]
            ).item()
            crop_start_y = torch.randint(
                0, label_hr.shape[1] - self.crop_size + 1, [1]
            ).item()
            crop_start_z = torch.randint(
                0, label_hr.shape[2] - self.crop_size + 1, [1]
            ).item()
            ...
            label_hr = label_hr[
                crop_start_x:crop_end_x,  # type: ignore[misc]
...
class OrthogonalSlices(data.Dataset):
    def __init__(self, labels_dir: Path, casenames: List[str], verbose: bool):
        ...
        image_sizes = [
            torch.tensor(label.shape) * spacing
...
class ImplicitDataset(OrthogonalSlices):
    ...
    def __init__(
        ...
        casenames: List[str],
        val_fraction: float,
        num_points_per_example_per_dim: int,
        slice_step_size: int,
        slice_step_axis: int,
        use_thick_slices: bool,
        do_yield_full_res: bool,
        ...
        gen = torch.Generator().manual_seed(SPRSF_START_MAIN_SEED)
        slice_start_ids = torch.randint(
            0, slice_step_size, [len(self.labels)], generator=gen
        ).tolist()
        results = [
            sparsen_volume(
                vol,
            ...
            for vol, spacing, offset, slice_start_id in zip(
                self.labels, self.spacings, self.offsets, slice_start_ids
            )
        ]
        ...
        if phase_type != PhaseType.INF:
            ...
            gen_val_sparsification = torch.Generator().manual_seed(
                SPRSF_START_SHARED_VAL_SEED
            )
            start_slice_ids = torch.randint(
                0, 2, [len(val_caseids)], generator=gen_val_sparsification
            ).tolist()
            ...
            for caseid, start_slice_id in zip(val_caseids, start_slice_ids):
                (
                    self.labels_sparse[caseid],
                    self.spacings_sparse[caseid],
        ...
        self.num_points_per_example = (
            num_points_per_example_per_dim ** self.image_size.numel()
            if num_points_per_example_per_dim > 0
            else -1
        )
    ...
    def __getitem__(self, item: int):
        ...
        if self.num_points_per_example > 0:  # use random subsample from sparse volume
            ...
            ndims = label.ndim
            voxel_ids = torch.empty(
                self.num_points_per_example, ndims, dtype=torch.int64
            )
            for i in range(ndims):
                voxel_ids[:, i] = torch.randint(
                    0, label.shape[i], [self.num_points_per_example]
                )
            label_values = label[voxel_ids[:, 0], voxel_ids[:, 1], voxel_ids[:, 2]]
            voxel_ids = voxel_ids.unsqueeze(1).unsqueeze(1)
            label_values = label_values.unsqueeze(1).unsqueeze(1)
            ...
            individual_voxel_ids = [
                torch.arange(num_elements) for num_elements in label.shape
            ]
            individual_voxel_ids_meshed = torch.meshgrid(
                individual_voxel_ids, indexing="ij"
            )
...
def create_data_loader(
    params: Dict, phase_type: PhaseType, verbose: bool
) -> data.DataLoader:
    ...
    if not labels_dir.exists() or not casefiles_basedir.exists():
        raise ValueError(
            "At least one of following data directories does not exist:"
    ...
    if task_type == config_io.TaskType.AD:
        if "sample_orthogonal_slices" in params and params["sample_orthogonal_slices"]:
            ...
            ds = ImplicitDataset(
                ...
                casenames,
                val_fraction,
                num_points_per_example_per_dim,
                slice_step_size,
                slice_step_axis,
                use_thick_slices,
                do_yield_full_res,
    ...
    dl = data.DataLoader(
        ds, batch_size, do_shuffle, num_workers=num_workers, drop_last=do_drop_last
    )
    return dl
