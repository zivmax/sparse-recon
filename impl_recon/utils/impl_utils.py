...
def sample_latents(
    ...
    occ_net: implicits.AutoDecoder,
    target_spatial_shape: torch.Tensor,
    target_spacings: torch.Tensor,
    batch_size_coords: int = 64**3,
) -> torch.Tensor:
    ...
    labels_pred = torch.empty(
        [batch_size_volumes, num_coords, 1, 1], dtype=torch.float32
    )
    for i in range(batch_size_volumes):
        ...
        coordinates = geometry_utils.generate_sampling_grid(
            target_spatial_shape, -1, 0.0, image_size, device, 1, False
        )
        coordinates = (
            coordinates.flatten(1, 3).unsqueeze(2).unsqueeze(2)
        )  # [1, N, 1, 1, 3]
        ...
        for j in range(num_batches_coords):
            ...
            with torch.no_grad():
                labels_pred[i, first_id:last_id] = (
                    occ_net(latents_curr, coordinates_curr).detach().cpu()[0]
                )
...
def eval_batch(
    ...
    if labels_pred.shape != labels_gt.shape:
        raise ValueError(
            f"Batch evaluation not possible: predicted shape {labels_pred.shape} "
    ...
    for j in range(batch_size):
        ...
        spacing = spacings[j]
        surf_distances = metrics.compute_surface_distances(
            label_gt, label_pred, spacing, True
        )
    ...
    if verbose:
        print(
            f"Batch ASD: {np.mean(asds[-batch_size:]):.2f}, "
