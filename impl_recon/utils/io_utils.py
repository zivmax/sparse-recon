...
def save_nifti_file(
    image_data: np.ndarray, local_to_global: np.ndarray, target_filepath: Path
):
...
def find_single_file(source_dir: Path, pattern: str) -> Path:
    ...
    if len(matched_files) != 1:
        raise ValueError(
            "The source directory has to contain exactly one file that matches the "
...
class RollingCheckpointWriter:
    ...
    def write_rolling_checkpoint(
        ...
        target_filepath = self.base_dir / "{}_{}.{}".format(
            self.base_filename, num_steps_trained, self.extension
        )
        torch.save(state, target_filepath)

        # Now delete oldest files until we reach the max
        paths = list(
            self.base_dir.glob("{}_*.{}".format(self.base_filename, self.extension))
        )
...
def load_latest_checkpoint(
    base_dir: Path, base_filename: str, extension: str = "pth", verbose: bool = False
) -> Tuple[OrderedDict, dict, int, int]:
    """
    ...
    if not base_dir.exists():
        raise ValueError(
            "Model directory for checkpoint loading does not exist:\n{}".format(
                base_dir
            )
        )
    ...
    if not paths:
        raise ValueError(
            "Model directory for checkpoint loading does not contain any "
            "checkpoints:\n{}".format(base_dir)
        )
