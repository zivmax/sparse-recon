...
def create_model(params: dict, image_size: Optional[torch.Tensor]) -> torch.nn.Module:
    ...
    if task_type == config_io.TaskType.AD:
        ...
        net = implicits.AutoDecoder(
            latent_dim, len(image_size), image_size, op_num_layers, op_coord_layers
        )
...
def train_one_epoch(
    ...
    if epoch % log_epoch_count == 0:
        ...
        if logger is not None:
            ...
            logger.add_scalar(
                "metric/train", metric_avg, global_step=num_epochs_trained
            )
            if lat_reg is not None:
                # Log average squared norm of latents
                logger.add_scalar(
                    "lat_norm2/train", lat_reg.item(), global_step=num_epochs_trained
                )
        ...
        if verbose:
            ...
            print(
                f"[{num_epochs_trained}/{num_epochs_target}] "
...
def optimize_latents(
    ...
    for i in range(num_iters):
        ...
        if (i + 1) % eval_every_x_steps == 0:
            ...
            if (
                verbose
                and round((i + 1) / eval_every_x_steps) % print_every_x_evals == 0
            ):
                print(
                    f"Step {i + 1:04d}/{num_iters:04d}: loss {loss.item():.4f} DSC {dsc:.3f} "
            ...
            if num_const_train_dsc == max_num_const_train_dsc:
                print(
                    f"Reached stopping critertion after {i + 1} steps. "
...
def main():
    ...
    model_dir = (
        model_basedir / params["model_name"]
    ...
    if model_dir is not None:
        if model_dir.exists():
            raise ValueError(
                "Model directory already exists. Exiting to prevent accidental "
                f"overwriting.\n{model_dir}"
            )
        model_dir.mkdir()
        ...
        checkpoint_writer = io_utils.RollingCheckpointWriter(
    ...
    ds_loader_train = data_generation.create_data_loader(
        params, data_generation.PhaseType.TRAIN, True
    )
    ds_loader_val = data_generation.create_data_loader(
        params, data_generation.PhaseType.VAL, True
    )
    image_size = (
    ...
    if task_type == config_io.TaskType.AD:
        ...
        latents_train = torch.nn.Parameter(
            torch.normal(
                0.0,
                1 / math.sqrt(latent_dim),
                [num_examples_train, latent_dim],
                device=device,
            )
        )
