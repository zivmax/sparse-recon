...
def find_closest_image_axes(ijk_to_lps: np.ndarray) -> tuple[list[int], list[int]]:
    ...
    if (
        closest_axis_to_x == closest_axis_to_y
        or closest_axis_to_y == closest_axis_to_z
        or closest_axis_to_z == closest_axis_to_x
    ):
        raise ValueError(
            f"Ambiguous closest axes after rotation: "
...
def normalize_coordinates(
    image_data: np.ndarray, ijk_to_ras: np.ndarray, verbose: bool
    ...
    if not np.isclose(det, 1.0) or not np.allclose(prod_with_t, np.eye(3), atol=1e-5):
        raise ValueError(
            f"Remaining matrix is not a rotation matrix. Determinant is {det}, "
            f"R^T @ R:\n{prod_with_t}"
        )

    if verbose:
        print(
            "closest signed image axes:",
            [ax * sign for ax, sign in zip(new_axes, axes_signs)],
        )
...
class DataGeneartor:
    ...
    def __getitem__(self, item) -> tuple[np.ndarray, np.ndarray, str]:
        ...
        if casename_match is None:
            raise ValueError(
                f"Filename does not match with casename pattern:\n{lbl_path.name}"
            )
...
def save_individual_vertebrae(
    ...
    for vrtbr_type in types_masked:
        ...
        if np.any(vertebra_size > bbox_size - 2):
            raise ValueError(
                f"Vertebra type {vrtbr_type} in case {casename} is larger than "
                f"bbox - 2: {vertebra_size} vs {bbox_size - 2}."
            )
        crop_lbl = crop_and_pad(labels_binary, position, bbox_size)
        save_nifti(
            crop_lbl,
            target_labels_dir / f"{casename}_{vrtbr_type:02d}.nii.gz",
            tuple(spacing_lbl),
        )
...
def main():
    ...
    for i, (labels, spacing_lbl, casename) in enumerate(data_gen):
        ...
        max_curr, avg_curr = save_individual_vertebrae(
            labels, spacing_lbl, bbox_size, target_labels_dir, casename
        )
